...
#' @export
#' @author Jenn Asimit
FLASHFMwithJAM <- function(beta1, corX, raf, ybar, N, r2 = 0.99, save.path,TOdds=1,covY,cpp=0.99,NCORES) {
 ...
 snpGroups <- makeSNPgroups2(main.input,fm.multi,is.snpmat=FALSE,min.mppi=.001, minsnpmppi=0.001,r2.minmerge=0.6)
...
#' @export
#' @author Jenn Asimit 
FLASHFMwithJAMhat <- function (beta1, corX, raf, ybar, N, r2 = 0.99, save.path, TOdds = 1, 
    covY, cpp = 0.99, NCORES) 
{
    M <- length(ybar)
    Vy <- diag(covY)
    main.input <- JAMexpandedCor.multi(beta1, corX, raf, ybar, 
        Vy, N, r2 = r2, save.path)
    gc(verbose = FALSE)
    ss.stats <- summaryStats(Xmat = FALSE, ybar.all = ybar, main.input = main.input)
